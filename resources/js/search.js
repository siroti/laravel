...
*/
        ...
        function atualizarTipos(data) {
            ...
            if (data && typeof data === 'object') {
                const savedTipo = localStorage.getItem('selected_tipo');
                let tipoValido = false; // Flag para verificar se o tipo salvo existe
                ...
                Object.keys(data).forEach((category) => {
                    ...
                    if (firstType) {
                        ...
                        // Verifica se o tipo salvo corresponde a este tipo
                        if (savedTipo === firstType) {
                            input.checked = true;
                            tipoValido = true; // Marca que encontrou o tipo
                            
                            // Resto da lógica para carregar subtipo...
                            let tentativas = 0;
                            const maxTentativas = 10;
                            function tentarCarregarSubtipo() {
                                if (Array.isArray(data[category]) && data[category].length > 0) {
                ...
                // Se o tipo salvo não existe nos tipos disponíveis, limpa
                if (savedTipo && !tipoValido) {
                    console.log("Tipo salvo inválido removido:", savedTipo);
                    localStorage.removeItem('selected_tipo');
                    if (tipoInput) {
                        tipoInput.value = '';
                    }
                }
                ...
                console.log('Nenhum tipo encontrado');
                // Se não há dados, limpa o campo também
                if (tipoInput) {
                    tipoInput.value = '';
                }
            }
        ...
        // Função para validar se o tipo atual é válido
        function validarTipoAtual() {
            const tipoInput = document.getElementById("b_tipo");
            if (!tipoInput) return;
            
            const tipoAtual = tipoInput.value;
            if (!tipoAtual || tipoAtual === '') return;
            
            // Verifica se existe um radio com esse valor
            const radioCorrespondente = document.querySelector(`input[name="tipo"][value="${tipoAtual}"]`);
            
            if (!radioCorrespondente) {
                console.log("Tipo inválido detectado e removido:", tipoAtual);
                tipoInput.value = '';
                localStorage.removeItem('selected_tipo');
                
                // Limpa subtipo também
                const subtipoInput = document.getElementById("b_subtipo");
                if (subtipoInput) {
        ...
        // Chama a validação após carregar os tipos
        setTimeout(() => {
            validarTipoAtual();
        }, 200);
        ...
        function AtualizarSelectTipo(){
            ...
            // Verifica se há algum tipo marcado
            ...
            if (tipoSelecionado && tipoSelecionado.nextElementSibling) {
                ...
                // Se não há tipo selecionado, verifica se o valor atual existe nos tipos disponíveis
                const tipoAtual = tipoInput.value;
                const tiposDisponiveis = Array.from(document.querySelectorAll('input[name="tipo"]'))
                    .map(radio => radio.value);
                
                // Se o valor atual não existe nos tipos disponíveis, limpa
                if (tipoAtual && !tiposDisponiveis.includes(tipoAtual)) {
                    tipoInput.value = '';
                    console.log("AtualizarSelectTipo - Tipo inválido removido:", tipoAtual);
                    // Remove do localStorage também
                    localStorage.removeItem('selected_tipo');
                    atualizarEstadoDormitorios();
                    console.log("atualizarEstadoDormitorios");
                } else if (!tipoAtual) {
                    tipoInput.value = '';
                    console.log("AtualizarSelectTipo - Nenhum tipo selecionado");
                    atualizarEstadoDormitorios();
                    console.log("atualizarEstadoDormitorios 2");
                }
            }
